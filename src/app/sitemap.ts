...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticItems: MetadataRoute.Sitemap = [
    { url: `${siteUrl}/`, changeFrequency: 'daily', priority: 1, lastModified: new Date() },
    { url: `${siteUrl}/bang-xep-hang`, changeFrequency: 'weekly', priority: 0.8, lastModified: new Date() },
  ];

  try {
    const movies = await getAllMovies();
    if (!Array.isArray(movies) || movies.length === 0) return staticItems;
